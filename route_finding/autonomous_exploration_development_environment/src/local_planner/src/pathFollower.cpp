...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubSpeed = nh.advertise<geometry_msgs::TwistStamped> (SpeedTopicName, 5);
  geometry_msgs::TwistStamped new_vel;
  new_vel.header.frame_id = "vehicle";
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      pubSkipCount--;
      if (pubSkipCount < 0) {
        new_vel.header.stamp = ros::Time().fromSec(odomTime);
        if (fabs(vehicleSpeed) <= maxAccel / 100.0) new_vel.twist.linear.x = 0;
        else new_vel.twist.linear.x = vehicleSpeed;
        new_vel.twist.angular.z = vehicleYawRate;
        pubSpeed.publish(new_vel);
