...
auto DJICRCHelper::VerifyCRC16Checksum(const uint8_t* pchMessage, size_t length) noexcept -> bool
{
	...
	// std::cout<<(wExpected & 0xff)<<' '<<(wExpected >> 8 & 0xffd)<<':'<<(int)pchMessage[length - 2]<<' '<<(int)pchMessage[length - 1]<<std::endl;
	return (wExpected & 0xff) == (int)pchMessage[length - 2] && (wExpected >> 8 & 0xff) == (int)pchMessage[length - 1];
}
